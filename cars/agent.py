...
class SimpleCarAgent(Agent):
    def __init__(self, history_data=int(50000)):
        ...
        self.neural_net = Network([self.rays + 4,(self.rays + 4) * 2,
    ...
    def from_weights(cls, layers, weights, biases):
        # Функция, присваивающая весам и смещениям значения, заданные в файле (указанном при запуске  скрипта) и выдающая исключения, если
        # эти данные были записаны некорректно. На основании этих данных создается нейросеть для конкретного агента.
        """
    ...
    def choose_action(self, sensor_info):
        ...
        #############################################################################
        # 
        # метод принимает на вход данные о положении агента и свойства машинки
        # затем для 9 доступных действий нейросетью считаем возможные награды
        # выбираем максимальную (а в 5% случаев случайную)
        # логируем входные и выходные данные 
        #
        #############################################################################
    ...
    def receive_feedback(self, reward, train_every=150, reward_depth=7):
        """
        ...
        i = -1
        ##############################
        # Непонятно что означает первое условие в цикле ниже
        ###############################
        ...
        if not self.evaluate_mode and (len(self.reward_history) >= train_every) and not (self.step % train_every):
            ...
            self.neural_net.SGD(training_data=train_data, epochs=15, mini_batch_size=train_every, eta=1)
